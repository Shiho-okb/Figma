...
FLOCSS設計、PCファースト、CSS変数対応、自動リロードなど開発環境を提供します。
...
- ✅ **FLOCSS設計** 採用
- ✅ **PCファースト設計**（デフォルトPC、SPはメディアクエリで上書き）
- ✅ **rem→pxフォールバック機能**（オプション、gulpfile.jsで設定）
- ✅ **インデント2スペース**で自動整形
- ✅ CDNファイル（`.min.css`や`src/css/`フォルダ）は整形処理をスキップ
- ✅ `**`ワイルドカードで自動インポート（`_index.scss`不要）
...
新しいファイルを追加しても`_index.scss`を編集する必要はありません。

## rem→pxフォールバック機能

古いブラウザ対応のため、rem値の前にpx値を自動追加できます。

### 設定方法

`gulp/gulpfile.js`の`REM_FALLBACK_ENABLED`を変更：

```javascript
const REM_FALLBACK_ENABLED = true;  // px + rem（フォールバックあり）
const REM_FALLBACK_ENABLED = false; // remのみ（デフォルト）
```

### 出力例

```css
/* REM_FALLBACK_ENABLED = true の場合 */
font-size: 20px;      /* フォールバック */
font-size: 1.25rem;

/* REM_FALLBACK_ENABLED = false の場合 */
font-size: 1.25rem;
```
...
- **PCファースト設計**
- **rem単位推奨**（pxフォールバック機能あり）
- **対応ブラウザ**: モダンブラウザのみ（IE11非対応）# Figma
